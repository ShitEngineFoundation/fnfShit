...
class ChartingState extends FlxUIState
{
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		stepperSectionBPM = new FlxUINumericStepper(10, 80, 1, Conductor.BPM, 0, 9999, 0);
	...
	function loadSong(daSong:String):Void
	{
		...
		FlxG.sound.playMusic(Paths.getSound("songs/" + daSong + '/sound/Inst', true), 0.6);

		// WONT WORK FOR TUTORIAL OR TEST SONG!!! REDO LATER
		vocals = new FlxSound().loadEmbedded(Paths.getSound("songs/" + daSong + '/sound/Voices', true));
	...
	/* this function got owned LOL
		function lengthBpmBullshit():Float
		{
			if (_song.notes[curSection].changeBPM)
				return16 * (_song.notes[curSection].bpm / _song.bpm);
			else
				return16;
	}*/
	...
	{
		var curStep = Math.floor(Conductor.stepCount);
		var curBeat = Math.floor(Conductor.beatCount);
		...
		FlxG.watch.addQuick('daBeat', Conductor.beatCount);
		FlxG.watch.addQuick('daStep', Conductor.stepCount);
		...
		{
			changeNoteSustain(Conductor.stepLength);
		}
	...
	function copySection(?sectionNum:Int = 1)
	{
		...
		{
			var strum = note[0] + Conductor.stepLength * (_song.notes[daSec].lengthInSteps * sectionNum);
	...
	function selectNote(note:Note):Void
	{
		...
		{
			if (i.strumTime == note.time && i.noteData % 4 == note.lane)
			{
